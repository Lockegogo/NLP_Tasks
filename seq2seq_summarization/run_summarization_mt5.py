...
sys.path.append("../")
from data_loader import LCSTS, get_dataLoader

from seq2seq_summarization.arg import parse_args
...
def test_loop(args, dataloader, model, tokenizer):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader):
            ...
            # 注意：这里给中文句子中的每个词之间加了空格，这样计算 ROUGE 时才能正确计算
...
def predict(args, document: str, model, tokenizer):
    inputs = tokenizer(
        document, max_length=args.max_input_length, truncation=True, return_tensors="pt"
    )
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.model_checkpoint, legacy=False)
