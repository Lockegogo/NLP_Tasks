...
from seq_labeling_ner.data_loader import CATEGORIES, PeopleDaily, get_dataLoader
...
def test_loop(args, dataloader, model):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader):
            ...
            # 计算每个序列的有效长度：对 attention mask 进行求和
            lens = np.sum(
                batch_data["batch_inputs"]["attention_mask"].cpu().numpy(), axis=-1
            )
            # 将 true_label 和 pred_label 还原为原始的标签序列 (seqeval.classification_report 接受的格式)
            # 还原时注意过滤特殊 token
            # 嵌套列表中的每一个列表长度都不相同
            true_labels += [
                [
                    ...
                    if idx > 0 and idx < seq_len - 1  # [CLS] 文本文本文本 [SEP] [PAD]
                ]  # 所以首尾不计入
...
def train(args, train_dataset, dev_dataset, model, tokenizer):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        logger.info(
            f"Dev: micro_F1 - {(100*micro_f1):0.4f} macro_f1 - {(100*macro_f1):0.4f} weighted_f1 - {(100*dev_f1):0.4f}"
        )
        if dev_f1 > best_f1:
            best_f1 = dev_f1
...
def test(args, test_dataset, model, tokenizer, save_weights: list):
    ...
    for save_weight in save_weights:
        ...
        micro_f1, macro_f1, dev_f1 = (
            metrics["micro avg"]["f1-score"],
            metrics["macro avg"]["f1-score"],
            metrics["weighted avg"]["f1-score"],
        )
        logger.info(
            f"Test: micro_F1 - {(100*micro_f1):0.4f} macro_f1 - {(100*macro_f1):0.4f} weighted_f1 - {(100*dev_f1):0.4f}"
        )
...
def predict(args, sentence: str, model, tokenizer):
    ...
    # offsets: 字符偏移映射，用于将模型的输出映射回原始文本的字符级别
    # squeeze(0)：将张量的第一个维度 batch_size 挤压掉，因为这里只处理单个句子
    offsets = inputs.pop("offset_mapping").squeeze(0)  # (seq_len, 2)
    ...
    with torch.no_grad():
        ...
        logits = outputs[1]
    probabilities = (
        torch.nn.functional.softmax(logits, dim=-1)[0].cpu().numpy().tolist()
    )
    predictions = logits.argmax(dim=-1)[0].cpu().numpy().tolist()

    # 处理模型输出
    pred_label = []
    # 从索引 1 开始，因为模型输出的第一个 token (索引为 0) 是 [CLS]
    idx = 1
    while idx < len(predictions) - 1:
        pred = predictions[idx]
        label = args.id2label[pred]  # 将预测标签转换为实际标签，如 B-LOC
        if label != "O":
            label = label[2:]  # Remove the B- or I-
            start, end = offsets[idx]  # 获取当前位置对应的字符起始位置和结束位置
            all_scores = [probabilities[idx][pred]]  # 存储当前位置的预测概率
            # 处理同一实体的后续标记
            while (
                idx + 1 < len(predictions) - 1
                and args.id2label[predictions[idx + 1]] == f"I-{label}"
            ):
                all_scores.append(probabilities[idx + 1][predictions[idx + 1]])
                _, end = offsets[idx + 1]  # 更新结束位置
                idx += 1  # 移动到下一个标记

            score = np.mean(all_scores).item()  # 计算所有标记的平均预测概率
            start, end = start.item(), end.item()  # 将起始和结束位置转换为标量值
            word = sentence[start:end]  # 获取从原始句子中提取的实体词
            pred_label.append(
                {
                    "entity_group": label,
                    "score": score,
