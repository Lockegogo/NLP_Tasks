...
def test_loop(args, dataloader, model):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader):
            ...
            outputs = model(**batch_data)
            logits = outputs[1]
            tags = model.crf.decode(
                logits, batch_data["batch_inputs"]["attention_mask"]
            )
...
def train(args, train_dataset, dev_dataset, model, tokenizer):
    ...
    no_decay = ["bias", "LayerNorm.weight"]  # 有一些参数的学习率不需要衰减
    ...
    optimizer_grouped_parameters = (
        [
            ...
            },
            {
                "params": [
                    p
                    for n, p in mlp_param_optimizer
                    if not any(nd in n for nd in no_decay)
                ],
                "weight_decay": args.weight_decay,
                "lr": args.learning_rate,
            ...
            {
                "params": [
                    p for n, p in mlp_param_optimizer if any(nd in n for nd in no_decay)
                ],
                "weight_decay": 0.0,
                "lr": args.learning_rate,
            },
            ...
            },
            {
                "params": [
                    p
                    for n, p in cls_param_optimizer
                    if not any(nd in n for nd in no_decay)
                ],
                "weight_decay": args.weight_decay,
                "lr": args.crf_learning_rate,
            ...
            {
                "params": [
                    p for n, p in cls_param_optimizer if any(nd in n for nd in no_decay)
                ],
                "weight_decay": 0.0,
                "lr": args.crf_learning_rate,
            },
