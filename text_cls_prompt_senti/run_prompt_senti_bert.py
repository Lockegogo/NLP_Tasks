...
from text_cls_prompt_senti.data_loader import (
    ChnSentiCorp,
    get_dataLoader,
    get_prompt,
    get_verbalizer,
)
from text_cls_prompt_senti.model import BertForPrompt
...
def train(args, train_dataset, dev_dataset, model, tokenizer, verbalizer):
    ...
    train_dataloader = get_dataLoader(
        args, train_dataset, tokenizer, verbalizer, shuffle=True
    )
    dev_dataloader = get_dataLoader(
        args, dev_dataset, tokenizer, verbalizer, shuffle=False
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        logger.info(
            f"Dev: micro_F1 - {(100*micro_f1):0.4f} macro_f1 - {(100*macro_f1):0.4f}"
        )
...
def test(args, test_dataset, model, tokenizer, verbalizer, save_weights: list):
    test_dataloader = get_dataLoader(
        args, test_dataset, tokenizer, verbalizer, shuffle=False
    )
    ...
    for save_weight in save_weights:
        ...
        logger.info(
            f"POS: {100*pos_p:>0.2f} / {100*pos_r:>0.2f} / {100*pos_f1:>0.2f}, NEG: {100*neg_p:>0.2f} / {100*neg_r:>0.2f} / {100*neg_f1:>0.2f}"
        )
        logger.info(
            f"Test: micro_F1 - {(100*micro_f1):0.4f} macro_f1 - {(100*macro_f1):0.4f}"
        )
...
if __name__ == "__main__":
    ...
    model = BertForPrompt.from_pretrained(args.model_checkpoint, config=config).to(
        args.device
    )
    if args.vtype == "virtual":  # virtual label words
        ...
        logger.info(
            f"initializing embeddings of {verbalizer['pos']['token']} and {verbalizer['neg']['token']}..."
        )
        with torch.no_grad():
    ...
    if args.do_predict:
        ...
        for save_weight in save_weights:
            ...
            for s_idx in tqdm(range(len(test_dataset))):
                ...
                pred_res = predict(
                    args, sample["comment"], model, tokenizer, verbalizer
                )
                results.append(
                    {
